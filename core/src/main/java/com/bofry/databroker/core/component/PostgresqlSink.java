...
@Log4j2
public class PostgresqlSink extends BaseSink implements IAgent {

    private static final long serialVersionUID = 7021235118277790898L;

    private final IObjectDescriptor descriptor;
    private volatile IDocumentWriter writer;

    public PostgresqlSink(SinkConfiguration conf) {
        super(conf);
        this.descriptor = new DescriptorDecorator(new PostgresqlDescriptor());
    }
    ...
    private void createWriter() {
        if (this.writer == null) {
            synchronized (this) {
                if (this.writer == null) {
                    PostgresqlWriterBuilder builder = new PostgresqlWriterBuilder();
