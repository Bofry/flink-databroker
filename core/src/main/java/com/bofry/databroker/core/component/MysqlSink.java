...
@Log4j2
public class MysqlSink extends BaseSink implements IAgent {
    ...
    public MysqlSink(SinkConfiguration conf) {
        super(conf);
        this.descriptor = new DescriptorDecorator(new MysqlDescriptor());
    }
    ...
    private void createWriter() {
        if (this.writer == null) {
            synchronized (this) {
                if (this.writer == null) {
                    MysqlWriterBuilder builder = new MysqlWriterBuilder();
